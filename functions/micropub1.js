...
// src/functions/micropub1.ts
__export(exports, {
  handler: () => handler
});

// src/functions/lib/micropub1/auth.ts
var import_node_fetch = __toModule(require_lib3());
function checkAuth(headers, context) {
  return __async(this, null, function* () {
    var _a;
    if (headers["x-auth"]) {
      console.debug("Looking for secret key");
      if (headers.authorization !== `Bearer ${process.env.MICROPUB_KEY}`) {
        throw new Error(`Secret mismatch. Got: ${(_a = headers.authorization) == null ? void 0 : _a.slice(0, 4)}`);
      }
      return;
    }
    console.debug("Checking token endpoint");
    const res = yield (0, import_node_fetch.default)("https://tokens.indieauth.com/token", {
      headers: {
    ...
    if (!res.ok) {
      console.log(`Unexpected response: ${yield res.text()}`);
...
// src/functions/micropub1.ts
var handler = (event, context, callback) => __async(void 0, null, function* () {
  const params = event.queryStringParameters;
  console.log("GOT REQUEST:", __spreadProps(__spreadValues({}, event.headers), { authorization: "XXXXX" }));
  try {
    yield checkAuth(event.headers, context);
  } catch (e) {
